<template>
    ...
    <div class="col-span-5 flex justify-center items-center">{{ conviction.action.content }}</div>
    <div class="col-span-1 flex justify-center items-center">{{ formattedGav }}</div>
    <div class="col-span-1 flex justify-center items-center">{{ conviction.action.post_search }}</div>
    ...
    <div class="col-span-2 flex justify-around items-center">
        <button class="text-white bg-red-500 p-2 rounded-md">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    data() {
        return {
            info: {
                modal: 'conviction',
                type: 'info',
                data: this.conviction.arrest
            }
        }
    },
    computed: {
        formattedGav() {
            return this.formatTime(this.conviction.action.gav)
        },
        formattedFederal() {
            return this.formatTime(this.conviction.action.federal)
        }
    },
    methods: {
        formatTime(data) {
